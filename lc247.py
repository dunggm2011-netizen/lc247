# =====================================================================
# LC79 MD5 BOT v3 — PANEL ADMIN + AUTO GỬI NHÓM
# API: api.tool247.fun/api/pred-log?game=lc79_md5&limit=50
# Tool: TOOL THANH DUY
...
def is_admin_chat(message_or_call):
    try:
...
def cmd_admin(message):
    ...
    kb.add(
        types.InlineKeyboardButton("🗑 Xóa key", callback_data="adm_xoakey_help"),
    ...
    kb.add(
        ...
        types.InlineKeyboardButton("👥 Số user", callback_data="adm_users"),
    )
    kb.add(
        types.InlineKeyboardButton("🟢 Bật nhóm", callback_data="adm_group_on"),
        types.InlineKeyboardButton("🔴 Tắt nhóm", callback_data="adm_group_off"),
    )
    kb.add(
        types.InlineKeyboardButton("📜 Log 10 phiên", callback_data="adm_log"),
        types.InlineKeyboardButton("🔄 Reset user data", callback_data="adm_reset_confirm"),
    )
    bot.reply_to(message, "⚙️ <b>PANEL ADMIN</b>\nChọn chức năng:", parse_mode="HTML", reply_markup=kb)
...
def on_admin_callback(c):
    ...
    if data == "adm_taokey":
        ...
        bot.send_message(
            ...
            "🔑 <b>TẠO KEY</b>\n\n"
            "Cú pháp:\n"
            "<code>/taokey</code> — 1 key vĩnh viễn\n"
            "<code>/taokey 5</code> — 5 key vĩnh viễn\n"
            "<code>/taokey 5 1d</code> — 5 key hạn 1 ngày\n"
            "<code>/taokey 1 1h</code> — 1 key hạn 1 giờ\n"
            "<code>/taokey 1 1m</code> — 1 key hạn 1 tháng",
            parse_mode="HTML"
        )

    elif data == "adm_listkey":
        bot.answer_callback_query(c.id)
        cmd_list_keys(c.message)

    elif data == "adm_xoakey_help":
        bot.answer_callback_query(c.id)
        bot.send_message(
            c.message.chat.id,
            "🗑 <b>XÓA KEY</b>\n\n"
            "<code>/xoakey ABC123</code> — xóa 1 key\n"
            "<code>/xoakey ABC123 DEF456</code> — xóa nhiều key\n"
            "<code>/xoakey all</code> — xóa tất cả",
            parse_mode="HTML"
        )

    elif data == "adm_clean":
        removed = clean_expired_keys()
        bot.answer_callback_query(c.id, f"Đã xóa {removed} key hết hạn.")
        ...
        text = (
            ...
            f"▶️ Đang theo dõi: <b>{running}</b>\n"
            f"🔑 Key chưa dùng: <b>{len(valid_keys)}</b>"
        )
        bot.send_message(c.message.chat.id, text, parse_mode="HTML")

    elif data == "adm_group_on":
        if CHAT_ID:
            user_data[CHAT_ID] = {"running": True, "last_phien": None, "activated": True}
            bot.answer_callback_query(c.id, "Đã bật gửi nhóm.")
            bot.send_message(c.message.chat.id, f"🟢 Đã BẬT gửi nhóm <code>{CHAT_ID}</code>", parse_mode="HTML")

    elif data == "adm_group_off":
        if CHAT_ID and CHAT_ID in user_data:
            user_data[CHAT_ID]["running"] = False
            bot.answer_callback_query(c.id, "Đã tắt gửi nhóm.")
            bot.send_message(c.message.chat.id, "🔴 Đã TẮT gửi nhóm.", parse_mode="HTML")

    elif data == "adm_log":
        bot.answer_callback_query(c.id)
        data_api = get_data()
        if not data_api:
            bot.send_message(c.message.chat.id, "❌ Không lấy được dữ liệu API.")
            return
        lines = ["📜 <b>10 PHIÊN GẦN NHẤT</b>\n"]
        for entry in data_api["lich_su"][:10]:
            phien = entry.get("phien", 0)
            ket_qua = to_txt(entry.get("ket_qua"))
            du_doan = to_txt(entry.get("du_doan"))
            conf = entry.get("do_tin_cay", 0)
            ...
            lines.append(f"{icon} #{phien}: {ket_qua} → đoán {du_doan} ({conf}%)")
        bot.send_message(c.message.chat.id, "\n".join(lines), parse_mode="HTML")

    elif data == "adm_reset_confirm":
        kb = types.InlineKeyboardMarkup()
        kb.add(
            types.InlineKeyboardButton("✅ Xác nhận", callback_data="adm_reset_do"),
            types.InlineKeyboardButton("❌ Hủy", callback_data="adm_reset_cancel"),
        )
        bot.answer_callback_query(c.id)
        bot.send_message(
            c.message.chat.id,
            "⚠️ <b>XÁC NHẬN RESET</b>\n\n"
            "Sẽ reset toàn bộ user_data (running, last_phien).\n"
            "Key vẫn giữ nguyên.",
        ...
        for chat_id in list(user_data.keys()):
            if chat_id in ADMIN_IDS or chat_id == CHAT_ID:
                continue
            user_data[chat_id]["running"] = False
            user_data[chat_id]["last_phien"] = None
        bot.send_message(c.message.chat.id, "✅ Đã reset user data.", parse_mode="HTML")

    elif data == "adm_reset_cancel":
...
def main():
    log.info("=" * 60)
    log.info("LC79 MD5 BOT v3 ĐANG CHẠY")
